...
The server SHALL select the speaker attribution strategy at session finalize time by calling `select_strategy(recordings)` on the meeting's persisted recordings, then `assign_speakers(...)` to determine the `speaker` value written to each `transcript_chunk` row before the chunk is emitted to the client. A session whose recording configuration is neither dual-channel (`{blackhole, mic}`) nor single-channel (exactly one recording) SHALL cause the server to emit `{"type": "error", "error_code": "session.invalid_speaker_configuration"}` and close the connection.
...
#### Scenario: Successful single-channel session emits speaker_cluster labels

- **GIVEN** an authenticated owner connects, sends `start_meeting`, only the microphone stream captures audio over a five-minute session containing two speakers, the user sends `end_meeting`
