...
type HunkDecision = "use_new" | "use_previous";
type HunkDecisions = Record<number, HunkDecision>;
              ...
              >
                ...
                <div className="flex justify-end gap-1 px-1">
                  <button
                    ...
                    className={
                      decision === "use_new"
                        ? "rounded border border-(--color-primary) bg-(--color-primary) px-2 py-0.5 text-xs text-(--color-primary-foreground)"
                        : "rounded border border-(--color-border) bg-(--color-card) px-2 py-0.5 text-xs text-(--color-foreground) hover:bg-(--color-muted)"
                    }
                    aria-pressed={decision === "use_new"}
                  >
                    {t("playbook.diff.useNew")}
                  </button>
                  <button
                    type="button"
                    ...
                    className={
                      decision === "use_previous"
                        ? "rounded border border-(--color-primary) bg-(--color-primary) px-2 py-0.5 text-xs text-(--color-primary-foreground)"
                        : "rounded border border-(--color-border) bg-(--color-card) px-2 py-0.5 text-xs text-(--color-foreground) hover:bg-(--color-muted)"
                    }
                    aria-pressed={decision === "use_previous"}
                  >
                    {t("playbook.diff.usePrevious")}
                  </button>
                </div>
              </div>
            );
