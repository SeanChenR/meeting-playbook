# meetings-calendar-view Specification

## Purpose

Defines a calendar visualisation of the authenticated user's meetings at the
route `/meetings/calendar`. The view renders meetings whose
`scheduled_start_at` is set on a month or week grid via `react-big-calendar`
(with the `date-fns` localizer, weeks starting on Monday). The same page
also lists meetings that have no scheduled start time in a collapsible
"Unscheduled" panel below the grid.

The calendar is read-and-navigate only: clicking an event card navigates to
that meeting's detail page; clicking an empty day cell navigates to the
new-meeting form with the date pre-filled (`/meetings/new?date=YYYY-MM-DD`,
default time `09:00`). Event color reflects meeting status (scheduled =
blue, in_progress = green, completed = grey) using existing Tailwind theme
tokens. The list page (`/meetings`) and this calendar page expose reciprocal
toggle buttons so users can switch between the two views without losing
session state or mutating any meeting data.
...
<!-- @trace
source: refactor-meetings-tabs-unified
updated: 2026-05-19
code:
